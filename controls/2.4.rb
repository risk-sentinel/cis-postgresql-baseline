...
control 'C-2.4' do
  ...
  if %w[rds_instance aurora_cluster].include?(deployment)
    describe 'AWS shared-responsibility inheritance (CIS 2.4 — ' + "Do Not Use a Cluster-Wide File for Password Storage" + ')' do
      it 'is satisfied by AWS-managed controls — ' + "AWS provides no shell access to the underlying RDS / Aurora host. ~/.pgpass and pg_service.conf exist only on operator-managed client workstations, not on the database server; server-side password-file risks this control addresses do not apply to AWS-managed engines" + ' (evidence: AWS SOC 2 Type II, AWS FedRAMP Moderate, AWS FedRAMP High, AWS ISO 27001; AWS Artifact: https://console.aws.amazon.com/artifact/)' do
