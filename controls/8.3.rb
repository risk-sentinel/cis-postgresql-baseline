...
control 'C-8.3' do
  ...
  describe 'AWS shared-responsibility inheritance' do
    it 'is satisfied by AWS-managed controls — control is satisfied by AWS under the shared-responsibility model. CIS 8.3 asserts file/directory permissions on UNIX sockets, external_pid_file, dynamic_library_path entries, and shared_preload_libraries paths — all of which live on the OS layer that AWS Aurora manages. The pg_settings query that surfaces the configured paths is part of the check, but the substantive permissions assertion runs on the host, which the consumer doesn\'t operate (evidence: AWS SOC 2 Type II, AWS FedRAMP Moderate, AWS FedRAMP High, AWS ISO 27001; AWS Artifact: https://console.aws.amazon.com/artifact/)' do
