...
control 'C-3.1.4' do
  ...
  describe 'AWS shared-responsibility inheritance' do
    it 'is satisfied by AWS-managed controls — control is satisfied by AWS under the shared-responsibility model. AWS manages the log file destination directory on the Aurora host; customer-facing log delivery is via CloudWatch Logs (see cis-aws-database CIS 2.5 / 3.9 for the operator-side log-export configuration check) (evidence: AWS SOC 2 Type II, AWS FedRAMP Moderate, AWS FedRAMP High, AWS ISO 27001; AWS Artifact: https://console.aws.amazon.com/artifact/)' do
