...
control 'C-7.5' do
  ...
  if %w[rds_instance aurora_cluster].include?(deployment)
    describe 'AWS shared-responsibility inheritance (CIS 7.5 — ' + "Ensure Streaming Replication Parameters are Configured Correctly" + ')' do
      it 'is satisfied by AWS-managed controls — ' + "Aurora uses a distributed storage layer rather than PostgreSQL streaming replication; max_wal_senders / wal_keep_segments / hot_standby_feedback are not operator-tunable. For standalone RDS PostgreSQL these parameters are AWS-managed and tied to the read-replica feature" + ' (evidence: AWS SOC 2 Type II, AWS FedRAMP Moderate, AWS FedRAMP High, AWS ISO 27001; AWS Artifact: https://console.aws.amazon.com/artifact/)' do
